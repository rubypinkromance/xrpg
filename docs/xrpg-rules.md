...
flowchart LR
    ...
    Dominance --> Supremacy
    Supremacy --> Mastery[Arcane Mastery]
    Ethereal --> Mastery
```
...
```mermaid
flowchart LR
    Mend --> Body[Strengthen Body]
    Mend --> Mind[Augment Mind]
    Body --> Morph
    Body --> Ice[Ice Path]
    Mind --> Fatigue
    Mind --> Ice
    Morph --> Wildshape[Wild Shape]
...
```mermaid
flowchart LR
    Air[Air Missile] --> Maelstrom
    Water[Water Missile] --> Maelstrom
    Air --> Clarity
    Water --> Clarity
...
flowchart LR
    ...
    Smite --> Lightning[Lightning Bolt]
```
