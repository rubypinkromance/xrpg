...
actually say, what's _missing_, and the best inferred readings to fold into the
...
much of the earlier _cleanup_ commit's restructuring — so some changes I first read
as "settled" were rolled back. The list below reflects what's _actually_ in the doc
now.

**Terminology (confirmed in current HEAD — carry forward)**

- **"attribute growth"** — the original PDF's "attribute _scaling_" was renamed
...
- TOC is **doctoc-generated** — don't hand-edit; re-run
  ...
  Damage + Sample Combat under _Encounters_, but in the body Damage + A Sample
  Combat now sit up near Resources. Re-run doctoc to resync.

**Structure — REVERTED to the original PDF layout (important signal)**

- `5cd216d` rolled back the _cleanup_ reorg. Current HEAD:
...
- **Takeaway:** the author is wary of aggressive restructuring and prefers to keep
  the original organization. For the rewrite, _propose_ opinionated structural
...
- **Inferred:** There is no "miss = nothing happens." Each contest is a single
  _simultaneous exchange_ where the **loser suffers the consequence**. The sample
  combat shows this: "Layla slashes… _while the troll tries to knock her away
  first_." She loses the clash, so the troll's blow lands. The damage isn't a
...
- Conversational, built for instant messaging: player vs Encounter Player, taking
  turns _describing_, but resolving via opposed rolls.
...
- Charisma vs Willpower — entice someone who _ought to resist_.
...
- **The contest roll** decides _who wins_ → `Level Die + attribute`. This is where
  **percentage** attribute bonuses/maluses from gear archetypes land.
- **Attack vs Defence** decides _how many Hitpoints the loser sheds_ → these are
...
brutally swingy/fast unless _both_ sides have Defence. The drawn-out grind is
...
standard roll." So `1d4` → `2d4` (results summed). It's an _operation_, not a
stored die.

### Scaling insight

- **Bonus dice are huge early, minor late.** Early the die _is_ the roll (attrs
...
- **Why not disadvantage?** (a) Subtract-a-die is mathematically symmetric with
  ...
  distribution-dependent penalty. (b) The doc _uses_ "advantage/disadvantage" by
...
- **Attack spells:** `target's Sorcery + average of the chosen die`. Caster picks
  ...
  the _target's_ Sorcery (punching through resistance).
...
1. Mana cost (above).
2. Must _know_ the spell (1 perk pt in-school, **3×** out-of-school; start w/ one
   school + one spell).
...
An attack spell _is_ your contest that round; a bonus spell adds a die to a contest
...
- **Why this reading:** the **Attrition** encounter type only works if winning a
  ...
  ground down. The mechanic _requires_ it.
- So each lost exchange = HP loss (gear-gated) + resource drain; and even _won_
...
- **If charmable — cleanest ruling (recommended):** your action sets your attribute
  (Charisma); the defender resists with the stat matching the _attack_ (Willpower),
  NOT a mismatched Cha-vs-Ath. So it's the defined **Charisma vs Willpower**
  contest. The troll's "swing" is just the _stakes of failure_.
...
- [ ] Wording: "irregardless" (Equip Load, ~line 89) → "regardless" (nonstandard word
      introduced during the cleanup pass). (Scott note: What was the original word used here? Should we switch to irrespective?)
